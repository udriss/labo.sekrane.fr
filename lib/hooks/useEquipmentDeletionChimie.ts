...
export const useEquipmentDeletion = () => {
  ...
  const confirmDeletion = async (deleteItems?: boolean) => {
    ...
    try {
      if (deleteState.type === 'category') {
        await equipmentServiceChimie.deleteCategory(deleteState.categoryId)
      } else if (deleteState.type === 'item' && deleteState.itemName) {
        await equipmentServiceChimie.deleteCustomEquipment(deleteState.categoryId, deleteState.itemName)
      }
      closeDeletionDialog()
      return true
    } catch (error) {
  ...
  const checkForDuplicates = async (newItem: any, equipmentTypes: any[]): Promise<boolean> => {
    const duplicates = equipmentServiceChimie.findDuplicates(newItem, equipmentTypes)
