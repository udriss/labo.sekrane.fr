// lib/services/equipmentServiceChimie.ts

export const equipmentServiceChimie = {
  ...
  async submitEquipment(formData: any, selectedCategory: string, selectedItem: any, getAllEquipmentTypes: () => any[]) {
    ...
    if (isNewCustomEquipment) {
      ...
      const typeResponse = await fetch('/api/chimie/equipment-types', {
        method: 'POST',
  ...
  async checkSimilarEquipment(equipmentName: string) {
    const response = await fetch(`/api/chimie/equipment-types/similar?name=${encodeURIComponent(equipmentName)}`);
